...
export function WatchCanvas({phase,discovered,condition,clueHint,devAssetLabel,onRotate,onReveal}:Props){const ref=useRef<HTMLDivElement>(null);const [angle,setAngle]=useState(0);const angleRef=useRef(0);const drag=useRef({active:false,x:0,angle:0});
 ...
 const onPointerDown=(e:PointerEvent)=>{if(phase!=='INSPECT')return;(e.currentTarget as HTMLElement).setPointerCapture(e.pointerId);drag.current={active:true,x:e.clientX,angle};};
 const onPointerMove=(e:PointerEvent)=>{if(!drag.current.active)return;const next=drag.current.angle+(e.clientX-drag.current.x)*.012;angleRef.current=next;setAngle(next);onRotate(next);};
