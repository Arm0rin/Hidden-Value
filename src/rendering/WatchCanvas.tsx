...
export function WatchCanvas({phase,discovered,condition,clueHint,devAssetLabel,showDevAsset=false,restoreActiveTool,onRotate,onRestoreStroke,onReveal}:Props) {
  ...
  const drag=useRef({active:false,x:0,y:0,angle:0});
  useEffect(()=>{
  ...
  return <div className={phase==='RESTORE'&&restoreActiveTool?'watch-stage is-restoring':'watch-stage'} ref={ref}
    ...
    onPointerDown={e=>{
      ...
      e.currentTarget.setPointerCapture(e.pointerId); drag.current={active:true,x:e.clientX,y:e.clientY,angle:angleRef.current};
    }}
    onPointerMove={e=>{
      ...
      if(phase==='RESTORE'){
        ...
        if(Math.hypot(e.clientX-drag.current.x,e.clientY-drag.current.y)>24){drag.current.x=e.clientX; drag.current.y=e.clientY; onRestoreStroke?.();}
