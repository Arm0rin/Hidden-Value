...
export interface ItemInstance { definitionId:string; discoveredClueIds:string[]; condition:number; completed:boolean; owned:boolean; bought:boolean; sold:boolean; activeTool?:RestorationTool; toolProgress:number; repairOptionId?:string; saleModeId?:string; saleValue?:number; saleCost?:number; purchaseValue?:number; inspectionStartedAt?:number; }
export interface AuctionRecord { itemId:string; value:number; success:boolean; }
export interface GameState { version:number; saveVersion:number; phase:FlowPhase; player:{level:number;xp:number;cash:number}; workshop:{level:number}; progression:{completedItems:number;completedItemIds:string[];rejectedItemIds:string[];discoveredClues:string[]}; tutorial:{currentStep:string;completed:boolean}; stats:{totalProfit:number;itemsBought:number;itemsSold:number;auctionAttempts:number;auctionWins:number;auctionRevenue:number;auctionHistory:AuctionRecord[]}; activeItemId:string|null; activeItem:ItemInstance|null; sessionStartedAt:number; }
