...
export type ItemRenderType = 'pocketWatch'|'camera'|'pixelBox';
...
export interface ItemDefinition { id:string; category:string; renderType:ItemRenderType; displayNameKey:string; purchasePrice:number; trueValue:number; startingCondition:number; startingValuation:{min:number;max:number}; clues:ClueDefinition[]; restorationSteps:RestorationStepDefinition[]; finalCondition:number; repairOptions?:RepairOptionDefinition[]; }
export interface ItemInstance { definitionId:string; discoveredClueIds:string[]; condition:number; completed:boolean; owned:boolean; bought:boolean; sold:boolean; activeTool?:RestorationTool; toolProgress:number; repairOptionId?:string; }
