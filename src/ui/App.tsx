  ...
  return <main className={'shell phase-'+phase.toLowerCase()}>
    ...
    <section className="scene">
      ...
      <section className="panel">
        ...
        {phase==='WORKSHOP'&&<><div className="stats"><div><span>{t('cash')}</span><b>{'$'+state.player.cash}</b></div><div><span>{t('level')}</span><b>{String(state.workshop.level).padStart(2,'0')}</b></div><div><span>{t('profitShort')}</span><b>{'$'+state.stats.totalProfit}</b></div></div><div className="workshop-meta"><div className="meta-heading"><span>{t('collection')}</span><b>{completedCount}/{itemSequence.length}</b></div><div className="collection-grid">{itemSequence.map(candidate=>{const found=state.progression.completedItemIds.includes(candidate.id);return <div className={found?'collection-item found':'collection-item'}><span>{found?'✓':'·'}</span><b>{t(candidate.displayNameKey)}</b></div>;})}</div><div className="collection-goal"><small>{t('nextGoal')}</small><span>{collectionGoal}</span></div></div>{completedCount>0&&<div className="completion-note">{t('completeHint')}</div>}</>}
      </section>
    </section>
