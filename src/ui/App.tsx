...
export function App({game}:{game:Game}) {
  ...
  const saleValue=definition.trueValue+(selectedRepair?.valueBonus??0);
  const profit=saleValue-price-repairCost;
  const devLabel=t(definition.renderType==='camera'?'devAssetCamera':definition.renderType==='pixelBox'?'devAssetPixelBox':'devAsset');
  ...
  return <main className={'shell phase-'+phase.toLowerCase()}>
    ...
    <section className="scene">
      ...
      <section className="panel">
        ...
        {(phase==='APPRAISE'||phase==='SELL')&&<><div className="receipt"><div><span>{t('bought')}</span><b>{'$'+price}</b></div><div><span>{t('costs')}</span><b>{'$'+repairCost}</b></div><div><span>{t('value')}</span><b>{'$'+saleValue}</b></div><div className="profit"><span>{t('profit')}</span><b>{'+$'+profit}</b></div></div><button className="primary" onClick={()=>phase==='APPRAISE'?act(()=>game.toSell()):act(()=>game.sell())}>{phase==='APPRAISE'?t('appraise'):t('sell')} {phase==='SELL'?'$'+saleValue:''}</button></>}
