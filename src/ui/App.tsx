...
export function App({game}:{game:Game}) {
  ...
  const profit=definition.trueValue-price;
  const devLabel=t(definition.renderType==='camera'?'devAssetCamera':'devAsset');
  ...
  return <main className={'shell phase-'+phase.toLowerCase()}>
    ...
    <section className="scene">
      ...
      <section className="panel">
        ...
        {phase==='RESTORE'&&<><div className="condition-row"><span>{t('condition')}</span><strong>{Math.round(condition)}%</strong></div><div className="tool-row"><button className={item?.activeTool==='brush'?'tool selected':'tool'} onClick={()=>game.selectTool('brush')}>✣<span>{t('brush')}</span></button><button className={item?.activeTool==='polish'?'tool selected':'tool'} disabled={condition<definition.restorationSteps[0].targetCondition} onClick={()=>game.selectTool('polish')}>✦<span>{t('polish')}</span></button></div><button className="primary" disabled={!item?.activeTool} onClick={()=>act(()=>game.stroke())}>{item?.activeTool?(item.activeTool==='brush'?t('brush'):t('polish'))+' · '+item.toolProgress:t('restoreHint')}</button></>}
        {(phase==='APPRAISE'||phase==='SELL')&&<><div className="receipt"><div><span>{t('bought')}</span><b>{'$'+price}</b></div><div><span>{t('costs')}</span><b>$0</b></div><div><span>{t('value')}</span><b>{'$'+definition.trueValue}</b></div><div className="profit"><span>{t('profit')}</span><b>{'+$'+profit}</b></div></div><button className="primary" onClick={()=>phase==='APPRAISE'?act(()=>game.toSell()):act(()=>game.sell())}>{phase==='APPRAISE'?t('appraise'):t('sell')} {phase==='SELL'?'$'+definition.trueValue:''}</button></>}
        {phase==='RESULT'&&<><div className="result"><span>{'+$'+profit}</span><small>{t('resultSummary').replace('$118','$'+definition.trueValue)}</small></div><button className="primary" onClick={()=>act(()=>game.next())}>{t('next')}</button></>}
        {phase==='WORKSHOP'&&<><div className="stats"><div><span>{t('cash')}</span><b>{'$'+state.player.cash}</b></div><div><span>{t('level')}</span><b>01</b></div><div><span>{t('profitShort')}</span><b>{'$'+state.stats.totalProfit}</b></div></div>{completedCount>0&&<div className="completion-note">{t('completeHint')}</div>}</>}
      </section>
    </section>
