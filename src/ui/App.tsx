...
export function App({game}:{game:Game}) {
  ...
  return <main className={`shell phase-${phase.toLowerCase()}`}>
    ...
    <section className="scene">
      ...
      <WatchCanvas phase={phase} discovered={clue} condition={condition} clueHint={t('clueTap')} devAssetLabel={t('devAsset')} onRotate={angle => { if (Math.abs(angle) > 1) game.analytics.track('item_rotated', {itemId:pocketWatch.id}); }} onReveal={() => act(() => game.revealClue())}/>
