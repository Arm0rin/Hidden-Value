  ...
  return <main className={'shell phase-'+phase.toLowerCase()}>
    ...
    <section className="scene">
      <div className="scene-copy">
        {(phase==='CLIENT'||phase==='DECISION'||phase==='RESULT')&&<CharacterCard index={sellerIndex<0?0:sellerIndex+1} receiver={phase==='RESULT'} phase={phase} itemId={definition.id} t={t}/>}
...
function CharacterCard({index,receiver,phase,itemId,t}:{index:number;receiver:boolean;phase:string;itemId:string;t:Translator}){const sellerKey=itemId==='pocket_watch_01'?'sellerWatch':itemId==='vintage_camera_01'?'sellerCamera':itemId==='fake_luxury_watch_01'?'sellerLuxury':itemId==='pixel_box_84'?'sellerConsole':'sellerPainting';const dialogue=receiver?(phase==='RESULT'?'receiverResult':'receiverWelcome'):sellerKey;return <div className="character-card"><div className={'character-portrait '+(receiver?'receiver':'')} style={{backgroundPosition:receiver?'0% 0%':`${(index%3)*50}% ${index>=3?'100%':'0%'}`}}/><div className="character-dialogue"><small>{t(receiver?'receiverName':'sellerName')}</small><p>{t(dialogue)}</p></div></div>;}
