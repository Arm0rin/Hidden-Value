...
export function App({game}:{game:Game}) {
  ...
  const [sellerQuestion,setSellerQuestion]=useState<string|null>(null);
  ...
  const inspectionLeft=item?.inspectionStartedAt?Math.max(0,35000-(clock-item.inspectionStartedAt)):35000;
  ...
  const price=item?.purchaseValue??definition.purchasePrice;
  ...
  const sellerIndex=itemSequence.findIndex(candidate=>candidate.id===definition.id);
  ...
  return <main className={'shell phase-'+phase.toLowerCase()}>
    ...
    <section className="scene">
      <div className="scene-copy">
        {(phase==='CLIENT'||phase==='DECISION'||phase==='RESULT')&&<CharacterCard index={sellerIndex<0?0:sellerIndex+1} receiver={phase==='RESULT'} phase={phase} itemId={definition.id} t={t} question={sellerQuestion} ask={setSellerQuestion}/>}
        ...
        {phase==='INSPECT'&&<><div className="eyebrow">{t('inspect')} · {Math.ceil(inspectionLeft/1000)}s</div><h2>{t('turn')}</h2><p className="muted">{nextClue?t('decisionHint'):t('valueFound')}</p></>}
      ...
      <section className="panel">
        ...
        {phase==='INSPECT'&&<><div className="valuation"><span>{t('valuation')}</span><strong>{'$'+valuation.knownMin+'–$'+valuation.knownMax}</strong></div>{foundClue&&<div className="clue-card"><small>{t(foundClue.labelKey)}</small><b>{foundClue.code}</b><span>{t(foundClue.valueKey)}</span></div>}<button className="primary" disabled={Boolean(nextClue&&inspectionLeft>0)} onClick={()=>act(()=>game.decideBack())}>{nextClue&&inspectionLeft>0?t('turn'):t('back')}</button></>}
        {phase==='DECISION'&&<><div className="valuation"><span>{t('valuation')}</span><strong>{'$'+valuation.knownMin+'–$'+valuation.knownMax}</strong></div><button className="primary" onClick={()=>act(()=>game.buy())}>{t('buy')} {'$'+price}</button>{clue&&<button className="secondary" onClick={()=>act(()=>game.buy(.1))}>{t('negotiate')} {'$'+Math.round(price*.9)}</button>}<button className="secondary" onClick={()=>act(()=>game.refuse())}>{t('refuse')}</button></>}
...
function CharacterCard({index,receiver,phase,itemId,t,question,ask}:{index:number;receiver:boolean;phase:string;itemId:string;t:Translator;question:string|null;ask:(value:string)=>void}){const sellerKey=itemId==='pocket_watch_01'?'sellerWatch':itemId==='vintage_camera_01'?'sellerCamera':itemId==='fake_luxury_watch_01'?'sellerLuxury':itemId==='pixel_box_84'?'sellerConsole':'sellerPainting';const sellerName=itemId==='pocket_watch_01'?'sellerNameMara':itemId==='vintage_camera_01'?'sellerNameJonas':itemId==='fake_luxury_watch_01'?'sellerNameElena':itemId==='pixel_box_84'?'sellerNameDima':'sellerNameNora';const dialogue=receiver?(phase==='RESULT'?'receiverResult':'receiverWelcome'):question??sellerKey;return <div className="character-card"><div className={'character-portrait '+(receiver?'receiver':'')} style={{backgroundPosition:receiver?'0% 0%':`${(index%3)*50}% ${index>=3?'100%':'0%'}`}}/><div className="character-dialogue"><small>{t(receiver?'receiverName':sellerName)}</small><p>{t(dialogue)}</p>{!receiver&&phase==='CLIENT'&&<div className="seller-questions"><button onClick={()=>ask('sellerQuestionAge')}>{t('askAge')}</button><button onClick={()=>ask('sellerQuestionReason')}>{t('askReason')}</button></div>}</div></div>;}
